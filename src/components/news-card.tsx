...
export function NewsCard({ article }: NewsCardProps) {
  const aiData = article.ai_processed_data;
  ...
  return (
    <Card className="flex flex-col h-full bg-card/50 hover:bg-card/90 transition-colors duration-300 transform hover:-translate-y-1 motion-reduce:transition-none motion-reduce:transform-none">
      ...
      <CardHeader>
        ...
        {aiData?.category && <Badge variant="secondary" className="w-fit">{aiData.category}</Badge>}
      </CardHeader>
      <CardContent className="flex-grow">
        <p className="text-sm text-muted-foreground line-clamp-4">
          {aiData?.summary || 'Summary being generated by AI...'}
        </p>
      </CardContent>
      <CardFooter className="flex-col items-start gap-4">
        <div className="flex flex-wrap gap-2">
            {aiData?.hashtags?.map(tag => (
